...
from data.dataset import MultimodalEvalDataset
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Evaluate 3dproject stage1 LoRA checkpoint")
    ...
    for i in range(len(dataset)):
        ...
        with torch.inference_mode():
            generated = model.generate(
                ...
                image_grid_thw=item["image_grid_thw"].unsqueeze(0).to("cuda"),
                do_sample=False,
        ...
        if (i + 1) % 20 == 0:
            print(f"processed {i + 1}/{len(dataset)} | correct={num_correct} | with_answer={pred_with_answer_tag}", flush=True)
